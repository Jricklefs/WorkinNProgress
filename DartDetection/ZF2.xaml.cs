...
{
    /// <summary>
    /// Interaction logic for ZF2.xaml
    /// </summary>
    public partial class ZF2 : Window
    {
        // ======================================
        ...
        private void LoadImageButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                originalImage = CvInvoke.Imread(openFileDialog.FileName, ImreadModes.Color);

                // ✅ Ensure image is loaded successfully
                if (originalImage == null || originalImage.IsEmpty)
                {
                    MessageBox.Show("Failed to load the image. Please try a different file.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                {
                    MessageBox.Show("Invalid image dimensions. Check IMAGE_WIDTH and IMAGE_HEIGHT.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }

                // ✅ Resize safely using a new Mat
                Mat resizedImage = new Mat();
                CvInvoke.Resize(originalImage, resizedImage, new System.Drawing.Size(IMAGE_WIDTH, IMAGE_HEIGHT));

                originalImage.Dispose(); // Free memory of the old image
                originalImage = resizedImage; // Assign resized image
        ...
        {
            using (Bitmap bitmap = MatToBitmap(mat)) // Convert Mat to Bitmap manually
            {
        ...
        public static Bitmap MatToBitmap(Mat mat)
        {
            ...
            try
            {
                using (Image<Bgr, byte> image = mat.ToImage<Bgr, byte>()) // Convert Mat to Image<Bgr, byte>
                {
                    // Convert byte[,,] to byte[]
                    byte[] imageData = new byte[image.Width * image.Height * 3]; // 3 channels (BGR)
                    Buffer.BlockCopy(image.Data, 0, imageData, 0, imageData.Length);
